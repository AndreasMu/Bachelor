...
#Komisch
a14=3858.911+2459.681*Squiggle-76.30093*(Squiggle**2)-348.6057*(Squiggle**3)-48.61703*(Squiggle**4)
...
#komisch
s=math.log10(Z)
a20=26.52091+81.78458*Squiggle+115.6058*(Squiggle**2)+76.33811*(Squiggle**3)+19.50698*(Squiggle**4)
...
#KOMISCH
a32=6.682518+0.2827718*Squiggle-0.07294429*(Squiggle**2)
...
#Calculation of a few different coefficients for the actual formula.
dA=min(a34/(a33**a35),a36/(a33**a37)) 
if(M<Mh):
    dL=0.0
elif(Mh<M<a33):
...
dB=(a38+a39*(2.0**3.5))/(a40*(2.0**3)+(2.0**a41))-1.0 
if(M<Mh):
    dR=0.0
elif(Mh<M<=a42):
...
else: #Between those two there is a linear interpolation so I take both RTMS, add them up and divide by two for approximation.
    RTMS=((a18+a19*M**a21)/(a20+M**a22)+(-0.08672073*M**3+a23*M**a26+a24*M**(a26+1.5))/(a25+M**5))/(2.0)
if(M<0.5):
...
t1=min(1.0,Age/thook)
t2=max(0.0,min(1.0,(Age-(1.0-0.01)*thook)/(0.01*thook)))
...
#Calculations of LZAMS and RZAMS
